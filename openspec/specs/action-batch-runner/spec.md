# action-batch-runner Specification

## Purpose
定义在单个 session 内顺序编排既有 MCP 动作的批处理契约，确保上层 agent / client 可以用更低往返完成确定性执行。
## Requirements
### Requirement: 批处理必须在单一 session 与 scope 中顺序执行

Wisp Hand 服务 MUST 通过 `wisp_hand.batch.run` 在单一 `session_id` 下顺序执行一组步骤，并保证所有步骤共享同一 session scope 与安全状态。
...
`wisp_hand.batch.run` 的步骤类型 MUST 只允许复用已定义的 `move`、`click`、`drag`、`scroll`、`type`、`press`、`wait` 与 `capture` 动作，而不是引入绕过现有校验的隐藏步骤。
