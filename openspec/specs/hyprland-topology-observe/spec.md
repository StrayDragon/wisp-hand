...
Wisp Hand 服务 MUST 通过 `wisp_hand.desktop.get_topology` 返回当前 Hyprland 会话的 monitors、workspaces、active workspace、active window 与 windows 列表，并保持字段结构稳定。

#### Scenario: Hyprland 环境下返回拓扑快照

- **WHEN** 客户端在受支持的 Hyprland 环境中调用 `wisp_hand.desktop.get_topology`
