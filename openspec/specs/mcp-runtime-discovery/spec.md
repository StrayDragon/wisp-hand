# mcp-runtime-discovery Specification

## Purpose
定义连接前与连接后的 runtime discovery/preflight 契约，使外部接入方能够在连接前发现阻塞项，并在连接后获取 live runtime 元数据用于诊断与关联。
...
- **WHEN** 操作方在有效配置、依赖完整且关键写路径可写的环境下执行 `wisp-hand doctor --json`
