...
- [x] 1.1 新增 `[coordinates]` 配置段(Pydantic schema + 默认值): `mode`, `cache_enabled`, `probe_region_size`, `min_confidence` 等
- [x] 1.2 定义坐标映射数据模型: `CoordinateMap`/`MonitorMap`/`Bounds`/`PixelRatio` 并支持 JSON 序列化
- [x] 1.3 实现 `topology_fingerprint` 生成与 `state_dir` 下的 cache 读写/失效逻辑

## 2. 坐标后端实现

- [x] 2.1 实现 `hyprctl-infer` 后端: 基于 monitor `x/y/width/height/scale` 推断 `layout_bounds`, 并计算置信度
- [x] 2.2 实现 `grim-probe` 后端: 使用 `grim` 被动探针反推 `physical_size` 与 `pixel_ratio`, 并产出 `layout_bounds`
- [x] 2.3 实现 `auto` 选择器: 依据 scale/置信度选择最合适后端; 支持强制后端且不可用时明确失败
- [x] 2.4 实现诊断级 `active-pointer-probe` 流程(不默认启用): 通过显式确认 + 用户框选安全 region 做一次 move 并回读 cursorpos 校验 extent
...
- [x] 4.1 新增 `examples/attempts/diagnose_coordinates.py`(或扩展现有 smoke): 输出坐标后端选择、每个 monitor 的推断/探针结果与比例校验
- [x] 4.2 补齐“用户需要手动操作”的验证指引: slurp 框选安全区域、运行主动校准、如何判断偏移
- [x] 4.3 更新 README/ROADMAP 附录或 docs: 给出 mixed-scale 推荐配置与 fallback 方案
