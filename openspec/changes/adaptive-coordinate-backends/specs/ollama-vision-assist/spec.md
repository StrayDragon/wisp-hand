...
Wisp Hand 服务 MUST 通过 `hand.vision.locate` 基于 `capture_id` 与文本目标返回候选区域列表.

为避免缩放/多显示器下的坐标歧义, `hand.vision.locate` 的输出 MUST 显式区分坐标系:

- `candidates_scope`: 候选区域(坐标系为 scope/layout px, 可直接用于 `hand.pointer.*` 的 scope 坐标)
