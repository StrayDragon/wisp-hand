...
- `hand.capabilities` 只能返回基础依赖矩阵，缺少版本、运行实例、transport、保留策略与写路径状态等接入方真正需要的运行元数据。
...
- 不为了平滑过渡保留旧配置路径、旧字段别名或双轨 discovery 接口；直接升级到新的运行契约。
...
- `hand.capabilities`：用于已连接客户端获取 live runtime 视角，在基础能力矩阵之上追加版本、`runtime_instance_id`、`started_at`、transport、保留策略摘要与其他运行元数据。

这样可以让“是否可启动”和“当前这个已启动实例是谁”分别有稳定入口，同时避免 CLI 与 MCP tool 各自维护一套不同的探测逻辑。

备选方案是只扩展 `hand.capabilities`。放弃原因是外部客户端在握手前就需要知道依赖和配置是否可用，仅靠 MCP tool 无法覆盖连接前自检。
...
这是一次 additive hardening，不改现有 MCP tool 名称，也不引入兼容层。迁移方式是：

1. 扩展配置 schema，新增 retention 与 discovery 相关配置，默认值直接对齐 beta 目标。
2. 更新默认配置模板与 README，统一外部接入方的安装/启动路径。
3. 为外部客户端文档明确新的 discovery 契约：连接前用 `wisp-hand doctor --json`，连接后用 `hand.capabilities` 读取 live runtime 元数据。
4. 以 `uv run pytest`、OpenSpec validate 与 transport smoke test 作为发布前校验。
